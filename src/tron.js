...
hexAddressToBase58 = (hexAddress) => {
 ...
 chain:'Ethereum'
  ...
  sendFunds=(amountFunded, receiverAddress, expectedAddress, secretHash, amountExpected, receiverEmailId)=>{
    this.hexAddressToBase58("4172cf8b0cdbfe538f94ba8e0a0c7229bdda2d3c34");
    this.setState({ loading: true })
    var pointCount=0;
...
this.state.htlc.methods.withdraw(4,"arnav",0).send({ from: this.state.account , shouldPollResponse: true}).then((res) => {
const txId = res["txCompleted"]
window.alert(txId)
//this.sendEmail(receiverEmailId, txId)
this.setState({loading:false});
},e=>{
