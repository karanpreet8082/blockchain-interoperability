...
import HTLC from "./res/HTLC_Eth_Tron.json"
...
hexAddressToBase58 = (hexAddress) => {
 ...
 chain:'Ethereum'
  ...
  sendFunds=(amountFunded, receiverAddress, expectedAddress, secretHash, amountExpected, receiverEmailId)=>{
    this.hexAddressToBase58(expectedAddress);
    this.setState({ loading: true })
    /*var pointCount=0;
...
/*if(chainName === "Ethereum" && this.state.chain === "Ethereum")
{
  if(this.checkEthAddress(receiverAddress) === false) { window.alert("Invalid Ethreum Receiver address."); this.setState({loading:false});return; }
  if(this.checkEthAddress(expectedAddress) === false) { window.alert("Invalid Ethreum Expected address.");this.setState({loading:false});return;}
  
}
else if(chainName === "Tron" && this.state.chain === "Ethereum")
{
  console.log(this.checkTronAddress(expectedAddress));
  if(this.checkEthAddress(receiverAddress) === false) { window.alert("Invalid Ethereum Receiver address.");this.setState({loading:false});return; }
  if(this.checkTronAddress(expectedAddress) === false) { window.alert("Invalid Tron Expected address.");this.setState({loading:false});return;}
}
else if(chainName === "Ethereum" && this.state.chain === "Tron")
{
  if(this.checkTronAddress(receiverAddress) === false){ window.alert("Invalid Tron Receiver address.");this.setState({loading:false});return;}
  if(this.checkEthAddress(expectedAddress) === false) { window.alert("Invalid Ethreum Expected address.");this.setState({loading:false});return;}
}
else if(chainName === "Tron" && this.state.chain === "Tron"){
  if(this.checkTronAddress(receiverAddress) === false){ window.alert("Invalid Tron Receiver address.");this.setState({loading:false});return;}
  if(this.checkTronAddress(expectedAddress) === false){ window.alert("Invalid Tron Expected address.");this.setState({loading:false});return;}
}*/
...
const lockTime = 900;
    if(this.state.chain === "Ethereum") {
this.state.htlc.methods.withdraw(receiverAddress, secretHash, expectedAddress, lockTime, amountExpected).send({ from: this.state.account , value:amountFunded}).then(result =>
{
